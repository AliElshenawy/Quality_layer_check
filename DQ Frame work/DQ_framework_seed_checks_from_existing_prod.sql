...
N'MOHEY_PROD_MIGRATION', N'Approved', N'DQ_FRAMEWORK', 1);
INSERT INTO @seed VALUES (N'Campaign', N'staging.campaign_latest', N'CAM-016', N'CUSTOM_SQL', NULL, N'LOW', N'Region__c listing (report-only): surfaces distinct populated regions for review; no controlled list',
N'SELECT [Id] AS record_id, [Region__c] AS exception_value, N''Region populated - review distinct values (report-only)'' AS exception_details, NULL AS etl_run_id FROM {{SOURCE_VIEW}} WHERE NULLIF(LTRIM(RTRIM([Region__c])), '''') IS NOT NULL',
N'MOHEY_PROD_MIGRATION', N'Approved', N'DQ_FRAMEWORK', 1);
